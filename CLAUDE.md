...
md에 규칙을 기록할 때는 특정 속성명·값·예시에만 국한되지 않도록 **원칙 중심**으로 쓴다. 예시는 원칙을 뒷받침하는 용도로만 사용하며, 예시에 없는 상황에도 같은 원칙을 적용할 수 있어야 한다.

---

## Git

- 커밋은 작업 단위가 일단락되면 **에이전트 판단으로 동의 없이 진행 가능** (2026-06-11 사용자 확정). 논리 단위로 나눠 커밋한다.
- push는 사용자 지시가 있을 때만.
