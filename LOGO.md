...
```html
<svg class="logo_urbanbrand {section}_logo" role="img" aria-label="도시브랜드연구소 로고">
    <use href="images/logo_urbanbrand.svg#logo_urbanbrand"></use>
</svg>
```

- 베이스 클래스 `logo_urbanbrand` + 섹션별 클래스(`gnb_logo`, `services_logo`, `footer_logo` 등) 2개를 함께 부여
...
```css
/* 베이스: 부모 크기 채움 */
.logo_urbanbrand { display: inline-block; width: 100%; height: 100%; }

/* GNB — 흰색 + hover (링크 컨테이너에 위치 지정) */
.gnb_inner > a {
    position: absolute; left: 50%; top: 21px; transform: translateX(-50%);
    width: 48px; height: 48px;
}
.gnb_logo { color: #fff; transition: color .2s; }
.gnb_inner > a:hover .gnb_logo { color: #ddd; }
...
```html
<svg class="logo_urbanbrand"><use href="images/logo_urbanbrand.svg#icon_arrow"></use></svg>
```
